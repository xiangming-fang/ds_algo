...
public class SN0109 {
    // BST是二叉搜索树
    public indi.xm.jy.leetcode.data_structure.TreeNode sortedListToBST(ListNode head) {
        if (head == null){
            return null;
        }
    ...
    public indi.xm.jy.leetcode.data_structure.TreeNode highBalance(int[] ints, int startIndex, int endIndex){
        if (startIndex > endIndex){
            return null;
        }
        int mid = (startIndex + endIndex)/2;
        indi.xm.jy.leetcode.data_structure.TreeNode treeNode = new indi.xm.jy.leetcode.data_structure.TreeNode(ints[mid]);
        treeNode.left = highBalance(ints,startIndex,mid - 1);
        treeNode.right = highBalance(ints,mid + 1,endIndex);
        return treeNode;
    }

    public indi.xm.jy.leetcode.data_structure.TreeNode sortedListToBST2(ListNode head) {
        return getBalanceByLinkedList(head,null);
    }

    public indi.xm.jy.leetcode.data_structure.TreeNode getBalanceByLinkedList(ListNode head, ListNode tail){
        ...
        ListNode mid = slow;
        indi.xm.jy.leetcode.data_structure.TreeNode root  = new TreeNode(mid.val);
